...
export async function listResults(req, res, next) { try { res.json({ success: true, data: await platformService.listResults() }); } catch (err) { next(err); } }
export async function createResult(req, res, next) { try { const result = await platformService.createResult(req.body); if (req.user?.id) await recordUserAuditLog({ userId: req.user.id, action: 'create_result', details: result.id, performedBy: req.user.id }); res.status(201).json({ success: true, data: result }); } catch (err) { next(err); } }
export async function updateResult(req, res, next) { try { const result = await platformService.updateResult(req.params.resultId, req.body); if (req.user?.id) await recordUserAuditLog({ userId: req.user.id, action: 'update_result', details: req.params.resultId, performedBy: req.user.id }); res.json({ success: true, data: result }); } catch (err) { next(err); } }
export async function approveResult(req, res, next) { try { const result = await platformService.approveResult(req.params.resultId, req.user.id); if (req.user?.id) await recordUserAuditLog({ userId: req.user.id, action: 'approve_result', details: req.params.resultId, performedBy: req.user.id }); res.json({ success: true, data: result }); } catch (err) { next(err); } }
export async function publishResult(req, res, next) { try { const result = await platformService.publishResult(req.params.resultId); if (req.user?.id) await recordUserAuditLog({ userId: req.user.id, action: 'publish_result', details: req.params.resultId, performedBy: req.user.id }); res.json({ success: true, data: result }); } catch (err) { next(err); } }
export async function lockResult(req, res, next) { try { const result = await platformService.lockResult(req.params.resultId); if (req.user?.id) await recordUserAuditLog({ userId: req.user.id, action: 'lock_result', details: req.params.resultId, performedBy: req.user.id }); res.json({ success: true, data: result }); } catch (err) { next(err); } }
export async function correctResult(req, res, next) { try { const result = await platformService.correctResult(req.params.resultId, req.body); if (req.user?.id) await recordUserAuditLog({ userId: req.user.id, action: 'correct_result', details: req.params.resultId, performedBy: req.user.id }); res.json({ success: true, data: result }); } catch (err) { next(err); } }
export async function getAcademicSummary(req, res, next) { try { const summary = await platformService.getAcademicSummary(req.params.studentId, req.params.sessionId); res.json({ success: true, data: summary }); } catch (err) { next(err); } }
...
export async function listNotifications(req, res, next) { try { res.json({ success: true, data: await platformService.listNotifications() }); } catch (err) { next(err); } }
