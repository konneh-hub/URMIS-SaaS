    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          ...
          if (!notifBody.success) setError(notifBody.message || 'Failed to load communication');
        }
      } catch (e) {
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <PageHeader
          ...
          title="Communication"
          description="Messaging and communication center."
          badge={<Badge tone="info">{notifications.length} messages</Badge>}
          actions={(
            <Button variant="primary" size="sm" onClick={() => setIsOpen(true)}>Send message</Button>
          )}
        />
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <MetricTile title="Templates" value={templates.length} description="Reusable templates" badgeTone="info" badge="Templates" />
        </div>

        <Card title="Message log" description="All communications sent through your institution.">
          {loading ? (
      ...
      <Dialog
        open={isOpen}
        title="Send message"
        description="Broadcast a message across your institution."
